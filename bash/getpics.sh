...
echo "====task1====="
echo "Downloading and extracting the tarfile"
# download a tarfile of pictures to our Pictures directory if it isn't already there - assumes you are online and testing it using test -f
test -f ~/Pictures/pics.tgz || wget -q -O ~/Pictures/pics.tgz http://zonzorp.net/pics.tgz

# using test for testing if file is there and then extract it using tar -xf option and  then remove the local copy of the zipfile
test -f ~/Pictures/pics.tgz && tar -xf ~/Pictures/pics.tgz && rm ~/Pictures/pics.tgz
